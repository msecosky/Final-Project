---
title: "Final Project"
author: "Mike Secosky"
date: "12/9/2019"
output: html_document
---



```{r}
library(DataComputing)
library(tidyverse)
```

Research Question: Has the amount of people over the age of 50 who at least occasionally use the internet increased since 2009?





```{r}
InternetUsage2019 <- read_csv("C:\\Users\\secos\\OneDrive\\Desktop\\College\\Stat 184\\January 8-February 7, 2019 - Core Trends Survey - CSV.csv")
```

```{r}
InternetUsage2009 <- read.csv("C:\\Users\\secos\\OneDrive\\Desktop\\College\\June 18-21, 2009 - Video Uploading - CSV.csv")
```

```{r}
glimpse(InternetUsage2009)
```


```{r}
glimpse(InternetUsage2019)
```

```{r}
Elderly2019 <-
InternetUsage2019 %>%
  select(age, sex, eminuse) %>%
  group_by(sex) %>%
  filter(age >= 50) %>%
  filter(eminuse == 1)%>%
  summarise(occuse19 = n())
```

```{r}
Elderly2009 <-
InternetUsage2009 %>%
  select(age, sex, neta) %>%
  group_by(sex) %>%
  filter(age >= 50) %>%
  filter(neta == 1) %>%
  summarise(occuse09 = n())
  
```

```{r}
InternetGrowth <-
full_join(Elderly2009, Elderly2019) %>%
  mutate(frequency = occuse09 + occuse19)
```

```{r}
Graphdata <-
  table(InternetGrowth$sex, InternetGrowth$occuse09, InternetGrowth$occuse19, dnn = c("Sex", "Occuse"))

Graphdata
```

barplot(Elderly2009, ylab="", xlab="Gears", main="Side-By-Side Bar Chart", col=c("turquoise4", "turquoise2", "turquoise" ), beside=TRUE, width=.3)
```

